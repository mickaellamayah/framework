...
import mesAnnotations.AnnotationPost;
...
public class FrontServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response, Method method)
            throws ServletException, IOException {
        response.setContentType("text/json");
        PrintWriter out = response.getWriter();
        try {
            Mapping mapping = mappings.get(this.removeParameter(request.getRequestURL().toString()));
            System.out.println("Mappings enregistrés : " + mappings.keySet());
            if (mapping == null) {
                throw new Exception("Aucun mapping trouvé pour l'URL : " + request.getRequestURL());
            }

            this.verifDoublant(request.getRequestURL().toString());

            Object result = this.minvoke(mapping, request);

            if (method.isAnnotationPresent(RestApiAnnotation.class)) {
                Gson gson = new Gson();
                String jsonResponse;
                ...
                if (result instanceof String) {
                    ...
                    throw new Exception("Résultat inattendu : " + result.getClass().getName());
                }
            }
        } catch (Exception e) {
...
²
    // sprint10
    private Method getMethodFromMapping(HttpServletRequest request) throws Exception {
        ...
        Mapping mapping = mappings.get(fullUrlString);
        ...
        throw new Exception("Méthode introuvable pour le mapping : " + mapping.getMethodName());
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter out = response.getWriter();
        try {
            Method method = getMethodFromMapping(request);
            if (method.isAnnotationPresent(AnnotationGet.class)) {
                processRequest(request, response, method);
            } else {
                throw new Exception("La methode doit être GET");
            }
        } catch (Exception e) {
            out.println("La methode doit être GET");
            e.printStackTrace();
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (method.isAnnotationPresent(AnnotationPost.class)) {
                ...
                throw new Exception("La methode doit être POST");
            }
        } catch (Exception e) {
            out.println("La methode doit être POST");
            e.printStackTrace();
        }
    }
    // sprint10fin
